                ...
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <div style="float:right;">
                            <button type="button" class="btn btn-primary" onClick="funcdeletefrompopup('managedeletepopup');">Delete</button>
                        </div>
                        <div style="clear:both;"></div>
                    </div>
                </div>
                    ...
                    <div class="navbar-collapse">
                        ...
                        <ul class="navbar-nav my-lg-0">
                            <li class="nav-item dropdown">
                                <a class="nav-link dropdown-toggle text-muted waves-effect waves-dark" href="logout.php"> <i class="fa fa-power-off"></i> Logout</a>
                            </li>
                        </ul>
                    </div>
                </nav>
            ...
            <div class="page-wrapper">
                <div class="row page-titles">
                    <div class="col-md-5 align-self-center">
                        <h3 class="text-themecolor">Internal transfer of goods</h3>
                    </div>
                ...
                <div class="container-fluid">
                    ...
                    @endif
                    <div class="row">
                        <form action="warehouseTransferSecondView" method="post">
                            {{ csrf_field() }}
                            <div class="col-12">
                                <h4>Please, select the start and the destination warehouse</h4>
                            </div>
                            <div class="col-sm-3">
                                <div class="form-group">
                                    <label>Form warehouse</label>
                                    <select name="from" id="" class="form-control">
                                        @foreach ($warehouses as $key => $w)
                                            <option value="{{$w->idwarehouse}}"
                                                @if(isset($from) && $w->idwarehouse==$from)
                                                    selected=""
                                                @endif
                                            >{{$w->shortname}}</option>
                                        @endforeach
                            ...
                            <div class="col-sm-3" style="padding-top: 25px;">
                                <input type="submit" name="find" value="Show the items" class="btn btn-info">
                            </div>
                        </form>
                    </div>
                    <div class="row">
                        <div class="col-12">
                            <div class="card">
                                <div class="card-body">
                                    <form action="confirmWarehouseTransfer" method="post">
                                        {{ csrf_field() }}
                                            ...
                                            <table id="myTable" class="table table-striped table-bordered" width="100%">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th>Kit (yes/no)</th>
                                                        @foreach ($warehouses as $key => $w)
                                                        <th>Quantity available in {{$w->shortname}}</th>
                                                        @endforeach
                                                        <th></th>                                       
                                                        <th>Notes</th>                                       
                                                    </tr>
                                                ...
                                                <tbody>
                                                    @if(isset($lagerstands))
                                                        <input type="text" name="from" style="display: none;" value="{{$from}}">
                                                        <input type="text" name="to"   style="display: none;"  value="{{$to}}">
                                                        @foreach ($lagerstands as $key => $row)
                                                            @if ($row->active=='Yes' && $row->virtualkit=='No')
                                                    <tr>
                                                        <td>{{$row->sort}}</td>
                                                        <td>{{$row->modelcode}}</td>
                                                        <td>{{$row->nameshort}}</td>
                                                        ...
                                                        @foreach ($warehouses as $key => $w)
                                                        <?php $idwarehouse = $w->idwarehouse; ?>
                                                        <td><b>{{$row->$idwarehouse}}</b></td>
                                                        @endforeach
                                                        <td><input type="text" class="form-control" name="idlagerstand[{{$row->idlagerstand}}]" value=""></td>
                                                        <td><input type="text" class="form-control" name="notes[{{$row->idlagerstand}}]" value=""></td>
                                                    </tr>
                                                            @endif
                                                        @endforeach
                                                    @endif
                                        ...
                                        <div class="form-group" style="float: right;">
                                            <input type="submit" name="transfer" value="Set items in shipping" class="btn btn-info">
                                        </div>
                                        @endif
                                    </form>
                                </div>
