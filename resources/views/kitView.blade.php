...
<html lang="en">
    ...
    <body class="fix-header fix-sidebar card-no-border">
        <div id="kitaddwarning" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="modal-standard-title" aria-hidden="true" style="margin-top:40px;">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-body text-center">
                        <p>Attention: You are creating more kit as you can. Do you want to create these kit(s)</p>
                    </div>
                    <div class="modal-footer">
                        <div style="float:left;"><button type="button" class="btn btn-default" data-dismiss="modal">Back</button></div>
                        <div style="float:right;"><button type="button" class="btn btn-primary" onClick="kitAddContinue();">Yes</button></div>
            ...
            <div class="page-wrapper">
                <div class="row page-titles">
                    <div class="col-md-5 align-self-center">
                        <h3 class="text-themecolor">View payment system</h3>
                    </div>
                            ...
                            <div class="card">
                                <div class="card-body">
                                    <div class="form-group">
                                        <label>Please select the destination to create kit</label>
                                        <select name="to" id="" class="form-control" onchange="getKitProducts(this)">
                                            <option value="">Select warehouse</option>
                                            @foreach ($warehouses as $key => $w)
                                                <option value="{{$w->idwarehouse}}" <?php if(Session::has('kitwarehouse') && Session::get('kitwarehouse') == $w->idwarehouse) echo "selected"; ?>>{{$w->shortname}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    @if(isset($products))
                                        ...
                                        <table id="myTable" class="table table-striped table-bordered" style="width: 100%;">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th>Article</th>
                                                    <th>Active (yes/no)</th>
                                                    <th>idwarehouse: {{$warehouselocation}}</th>
                                                    <th>Buffer</th>
                                                    <th>Kit to create</th>
                                                    <th></th> 
                                                    <th>Max Kit</th>
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="td-field">
                                                        <span  class="field-value">{{$row->warehousebuffer}}</span>
                                                        <div class="field-edit">
                                                            <input type="text" name="warehousebuffer" class="form-control" value="{{$row->warehousebuffer}}" data-id="{{$row->productid}}">
                                                        </div>
                                                    </td>
                                                    <td>
                                                        <input type="hidden" name="maxKit" id="maxKit_{{$row->modelcode}}" value="{{$row->maxKit}}">
                                                        <input type="text" name="newCount" id="newCount_{{$row->modelcode}}" value="">
                                                    </td>
                                                    <td>
                                                        @if($row->maxKit > 0) 
                                                    <button type="button" class="btn btn-primary" onclick="sendNewKit({{$row->modelcode}})">Send</button>
                                                        @endif
                                                    </td>
                                                    @if(($row->warehouseqty - $row->warehousebuffer) >= 0)
                                                    <td style="background: green; color: white;">{{$row->maxKit}}</td>
                                                    @else 
                                                        @if(($row->warehousebuffer - $row->warehouseqty) > $row->maxKit)
                                                        <td style="background: red; color: white;">{{$row->maxKit}}</td>
                                                        @else
                                                        <td style="background: yellow;">{{$row->maxKit}}</td>
                                                        @endif
                                                    @endif
                                                    <td>{{$row->pcs1}}</td> 
                                                    <td>{{$row->pcs2}}</td>
        ...
        <script>
            ...
            $(document).ready(function() {
                ...
                $('.field-edit .form-control').change(function(event) {
                    ...
                    label.show();
                    var data={productid:$(this).data('id'), buffer:$(this).val()};
                    console.log(data);
                    $.ajax({
                        url    : 'bufferUpdate',
                        data   : data,
            ...
            function getKitProducts(element) {
                var warehouse = element.value;
                location.href="createKit?warehouse="+warehouse;
            }

            function sendNewKit(modelcode) {
                var newCount = document.getElementById("newCount_"+modelcode).value;
                var maxKit   = document.getElementById("maxKit_"+modelcode).value;

                if(parseInt(newCount) > parseInt(maxKit)) {
                    tempmodelcode = modelcode;
                    $("#kitaddwarning").modal("show");
                } else {
                    var data = {
                        newCount : newCount,
                        modelcode : modelcode
            ...
            function kitAddContinue() {
                var newCount = document.getElementById("newCount_"+tempmodelcode).value;
                var maxKit   = document.getElementById("maxKit_"+tempmodelcode).value;

                var data = {
                    newCount    : newCount,
                    modelcode   : tempmodelcode
                }
