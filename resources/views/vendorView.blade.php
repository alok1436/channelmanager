            ...
            <div class="page-wrapper">
                <div class="row page-titles">
                    <div class="col-md-5 align-self-center">
                        <h3 class="text-themecolor">Order Vendor</h3>
                    </div>
                ...
                <div class="container-fluid">
                    ...
                    @endif
                    @if(Session::has('nonExistLocation'))
                        <?php $nonExistLocation = Session::get('nonExistLocation'); ?>
                        <div class='alert alert-danger'>
                            <button type='button' class='close' data-dismiss='alert'>&times;</button>
                            @foreach($nonExistLocation as $row)
                                @if($row != "")
                                <i class='fa fa-exclamation-circle'></i> 
                                Warning: You need to select location for {{$row}}.<br>
                                @endif
                            @endforeach
                            <?php Session::forget('nonExistLocation'); ?>
                        </div>
                    @endif
                    <div class="row">
                        <div class="col-12">
                            <div class="card">
                                <div class="card-body">
                                    <div class="col-sm-3">
                                        <input type="button" class="btn btn-info btn-block send_courier" data-toggle="modal" data-target="#myModalNew" value="Upload new Orders" name="">
                                                ...
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-body">
                                                        <table class="table table-responsive w-100" border="1">
                                                            ...
                                                            <form class="form-horizontal" action="importVendorFile" method="post" name="frmCSVImport" id="frmCSVImport" enctype="multipart/form-data">
                                                            {{csrf_field()}}
                                                            <tbody class="w-100">
                                    ...
                                    <div class="col-sm-3">
                                        <a href="sendvendororder" class="btn btn-info btn-block send_courier">Send</a>
                                    </div>
                                        ...
                                        <table id="myTable" class="table table-striped table-bordered" style="width: 100%;">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    <td>{{$row->idvendor}}</td>
                                                    <td>{{$row->shortname}}</td>
                                                    <td>Amazon - {{$row->location}}</td>
                                                    <td>{{$row->skuvendor}}</td>
                                                    <td>{{ \Illuminate\Support\Str::limit($row->title, 20, $end='...') }}</td>
                                                    <td>{{$row->asinvendor}}</td>
                                                    <td>{{$row->eanvendor}}</td>
                                                    <td>{{$row->skuvendor}}</td>
                                                    <td>{{$row->quantitySubmitted}}</td>
                                                    <td>{{$row->cost}}</td>
                                                    <td class="td-field">
                                                        <span class="field-value"> @if($row->send == 1) Yes @else No @endif</span>
                                                        <div class="field-edit">
                                                            <select name="send" class="form-control" data-id="{{$row->idvendor}}" style="width: 100%;" data-field="send">
                                                                <option value="">Select status</option>
                                                                <option value="1" @if($row->send == 1) selected @endif>Yes</option>
                                                                <option value="0" @if($row->send == 0) selected @endif>No</option>
                                                            </select>
                                                        </div>
                                                    </td>
                                                    <td class="td-field">
                                                        <span class="field-value" id="id_span_accepted_quantity_{{$row->idvendor}}">{{$row->accepted_quantity}}</span>
                                                        <div class="field-edit">
                                                            <input type="text" name="accepted_quantity" class="form-control" id="id_input_accepted_quantity_{{$row->idvendor}}" value="{{$row->accepted_quantity}}" data-id="{{$row->idvendor}}" data-field="accepted_quantity">
                                                        </div>
                                                    </td>
                                                    <td style="text-align: center">
                                                        @if($row->quantitySubmitted > $row->warehouseQnt)
                ...
                $('.field-edit .form-control').change(function(event) {
                    ...
                    $.ajax({
                        ...
                        success: function(result) {
                            ...
                            document.getElementById("id_span_accepted_quantity_"+response.idvendor).innerHTML = response.accepted_quantity;
                            document.getElementById("id_input_accepted_quantity_"+response.idvendor).value = response.accepted_quantity;
                        }
                    });
                });
