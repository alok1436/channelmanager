...
class MWSClient{
    ...
    public function ListOrders(DateTime $from, $allMarketplaces = true, $states = [
        'Unshipped', 'PartiallyShipped'
    ], $FulfillmentChannels = ['MFN'])
    {
        $query = [
     ...
     * @return array feed submission result
     */
    public function updateOrder(array $array)
    {
        $feed = [
        ...
        foreach ($array as $order) {
            $feed['Message'][] = [
                ...
                'OperationType' => 'Update',
                'OrderFulfillment' => [
                    'AmazonOrderID' => $order->idorderplatform,
                    'FulfillmentDate' => gmdate(self::DATE_FORMAT, $date->getTimestamp()),
                    'FulfillmentData' => [
                        'CarrierCode'           => $order->carriername,
                        'ShippingMethod'        => $order->carriername,
                        'ShipperTrackingNumber' => $order->tracking
                    ]
                ]
            ];
        }
    ...
    private function request($endPoint, array $query = [], $body = null, $raw = false)
    {
        ...
        try{
            ...
            // print_r($response->getHeader('x-mws-quota-max')[0]."--------------".$response->getHeader('x-mws-quota-remaining')[0]."--------------".$response->getHeader('x-mws-quota-resetsOn')[0]."<br>");
